import { amuletBroadcastProject, amuletKnowledge, createScenes, imagePrompt, markdown, sceneSafety, seedProject, videoPrompt } from "./data.js";
...
function isAmuletCategory() { return state.category === "พระเครื่อง" || state.category === amuletKnowledge.category; }
function header() {
...
function projectControls() {
  ...
  const amuletGuide = isAmuletCategory() ? `<article class="category-knowledge"><span class="tag">หมวดเฉพาะ</span><strong>${amuletKnowledge.title}</strong><p>ใช้มุม Macro โฟกัสมือ องค์พระ และผิววัสดุ โดยไม่ต้องมีใบหน้าหรือร่างกายอยู่ในเฟรม</p><ul class="knowledge-rule-list">${amuletKnowledge.rules.map((rule) => `<li>${rule}</li>`).join("")}</ul><button class="btn btn-small btn-primary" data-action="use-amulet-example">${icon("wand")} โหลดตัวอย่างพร้อมใช้ 3 ซีน</button></article>` : "";
  const knowledge = `<div class="knowledge-list"><article><strong>1. Hook ใน 2 วินาที</strong><p>เริ่มด้วยความรู้สึกหรือปัญหาจริง ไม่ใช่คำรับประกันผลลัพธ์</p></article><article><strong>2. 1 ซีน = 1 ความคิด</strong><p>ให้คนดูเข้าใจทัน แม้ดูแบบปิดเสียงหรือข้ามเข้ากลางคลิป</p></article><article><strong>3. Caption กับเสียงไม่ใช่สิ่งเดียวกัน</strong><p>Subtitle ใช้ข้อความสะกดถูก ส่วนคำอ่านมีไว้ให้ TTS และ Lip-sync เท่านั้น</p></article><article><strong>4. ลายน้ำต้องวางหลังสร้างวิดีโอ</strong><p>ไม่ส่งให้โมเดลวาดตัวอักษรไทย ลดปัญหาสะกดเพี้ยนและโลโก้ผิดรูป</p></article>${amuletGuide}</div>`;
  return `<aside class="control-rail"><div class="rail-intro"><p class="eyebrow">ทำทุกอย่างในหน้าเดียว</p><h1>เริ่มไว แล้วค่อย<br>เปิดรายละเอียด</h1><p>เลือกสิ่งจำเป็นก่อน ระบบจะจัด Prompt ให้พร้อมใช้งานทันที</p></div>${details("project", "สินค้าและเป้าหมาย", project, true, `${state.category} · ${state.platform}`)}${details("character", "ตัวละครและฉาก", character, false, "ล็อกความต่อเนื่อง")}${details("brand", "ชื่อเว็บไซต์และลายน้ำ", brand, true, state.brand.speakName || state.brand.watermarkEnabled ? "เปิดใช้แล้ว" : "เลือกได้")}${details("knowledge", "คู่มือทำคลิปให้น่าดู", knowledge, isAmuletCategory(), isAmuletCategory() ? "พระเครื่อง · 3 ซีน" : "4 เคล็ดลับ")}</aside>`;
}
...
function bind() {
  ...
  document.querySelectorAll("[data-category]").forEach((button) => button.addEventListener("click", () => { state.category = button.dataset.category; if (isAmuletCategory()) accordionOpen.knowledge = true; save(); render(); toast(`เลือกหมวด ${state.category} แล้ว`); }));
...
function action(name) {
  ...
  if (name === "use-amulet-example") { state = amuletBroadcastProject(); selectedId = state.scenes[0].id; selectedScenes.clear(); save(); render(); toast("โหลดตัวอย่างพระเครื่อง 3 ซีนแล้ว — ตรวจเลขจริงก่อนวางในวิดีโอ"); }
